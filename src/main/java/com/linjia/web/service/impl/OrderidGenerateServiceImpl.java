...
public class OrderidGenerateServiceImpl extends BaseServiceImpl<OrderidGenerate, Long> implements OrderidGenerateService {
	...
	/**************************订单号和订单组号生成规则******************************/
	/*
	 * type:1订单号；2订单组号；3退款单号;4积分订单号;5拼团订单号
	...
	private final String PINTUAN_ORDER_TYPE = "5";
	 ...
	 */
	@Override
	public long generatePintuanOrderId() {
		StringBuilder sb = new StringBuilder();
		OrderidGenerate model = new OrderidGenerate();
		mapper.insertSelective(model);
		sb.append(DateComFunc.formatDate(new Date(), format)).append(PINTUAN_ORDER_TYPE).append(model.getId());
