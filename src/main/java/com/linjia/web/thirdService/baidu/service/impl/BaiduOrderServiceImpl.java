...
//			bdData.getOrder().setPay_status(2);
			...
			//查询是否已存在
			int num=mapper.selectBdOrderNum(orderId);
			if(num>0){
				ordernum=1;
			}else{
				ordernum=mapper.insertBdOrder(bdData);
			}
			int innum=0;
			int disnum=0;
			if(ordernum>0&&num<1){
				...
				//优惠信息
				List<Discount> discounts=bdData.getDiscount();
				if(discounts!=null){
//					for (Discount discount : discounts) {
//						discount.setOrderId(orderId);
//						boolean disflag=mapper.insertBdDiscount(discount);
//						if(disflag){
//							disnum++;
//						}
//					}
	...
	@Override
	public boolean updateOrderStatus(Long order_Id, int status,String Authorization) {
		String orderId=String.valueOf(order_Id);
		//准备body数据
		Order order = new Order();
		order.setOrder_id(orderId);
		...
		boolean flag=false;
		try{
			sourceOrder=mapper.updateBdOrderStatus(order);
			if(sourceOrder>0){
				flag=true;
			}
		}catch(Exception e){
